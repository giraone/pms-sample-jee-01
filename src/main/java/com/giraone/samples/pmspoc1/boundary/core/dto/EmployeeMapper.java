...
public interface EmployeeMapper
{
	...
	@Mappings({
		...
		@Mapping(target = Employee_.DTO_NAME_religion, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_maritalStatus, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_numberOfChildren, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_countryOfBirth, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_birthPlace, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_birthName, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactEmailAddress1, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactEmailAddress2, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactPhone1, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactPhone2, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactFax1, ignore = true),
		@Mapping(target = Employee_.DTO_NAME_contactFax2, ignore = true) })
	void updateEntityFromDto(EmployeeDTO dto, @MappingTarget Employee entity);

	@Mappings({ @Mapping(target = "postalAddresses", ignore = true), // Is performed manually
	})
